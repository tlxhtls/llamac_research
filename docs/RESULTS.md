...
- Current PPG-only SOTA within this repo's tuned tabular suite is **ppg_rich + logistic_regression** with macro F1 **0.2353** and top-1 **0.2520** on grouped reported-emotion CV.

## Paper-style and baseline LightGBM checks

| Run | Model | Variant | Target | Split | Rows | Features n | Top-1 | Top-2 | Top-3 | Macro F1 | Balanced acc | Kappa |
|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
...
| optuna locked | logistic_regression | ppg_rich | reported | grouped | 5400 | 98 | 0.2520 | 0.4457 | 0.6256 | 0.2353 | 0.2388 | 0.0539 |
| optuna locked | lightgbm | ppg_rich | reported | grouped | 5400 | 98 | 0.2385 | 0.4356 | 0.6261 | 0.2298 | 0.2312 | 0.0414 |
...
uv run --group ml python scripts/build_features.py --mode ppg_rich --workers 8 --output data/processed/features_ppg_rich.parquet
...
uv run --group ml python scripts/tune_models.py --features data/processed/features_ppg_rich.parquet --feature-set ppg --target reported --split grouped --folds 5 --models lightgbm logistic_regression --trials 20 --metric macro_f1 --device cpu --output-dir artifacts/optuna/ppg_rich_reported_grouped
uv run python scripts/summarize_results.py artifacts/results artifacts/optuna/ppg_reported_grouped/locked-results artifacts/optuna/ppg_rich_reported_grouped/locked-results --output artifacts/results/summary_with_rich.csv
```

## Next research extensions

- Add leakage stress tests grouped by stimulus/video identity.
- Add nested or repeated grouped validation for the best PPG-rich configuration.
